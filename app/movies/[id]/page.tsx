    ...
    <div className="container-max">
      <article className="surface g-border overflow-hidden">
        <div className="relative h-[42vh] min-h-[260px]">
          {bg ? <Image src={bg} alt={movie.title} fill className="object-cover" priority /> : <div className="h-full skel" />}
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, var(--bg) 15%, transparent)" }} />
        </div>

        <div className="grid grid-cols-1 gap-6 p-5 sm:grid-cols-[auto,1fr]">
          <div className="row-start-1 sm:row-span-2 justify-self-center sm:justify-self-start -mt-16">
            <div className="g-border surface overflow-hidden" style={{ width: 160, height: 240 }}>
              {poster ? (
                <Image src={poster} alt={movie.title} width={160} height={240} className="h-full w-full object-cover" />
              ) : (
          ...
          <div>
            ...
            <p style={{ color: "var(--muted)" }}>
              Rilis: {formatDate(movie.release_date)} · Durasi: {movie.runtime ? `${movie.runtime} mnt` : "-"} · ⭐
